...
## References
- `references/android-apk-manual-build.md` — Build Android APKs without Gradle using SDK command-line tools (aapt2/javac/d8/zipalign/apksigner). Useful for WebView wrapper apps when Gradle dependency resolution fails or times out.
...
### Telegram Gateway Setup (Manual / Headless)

When `hermes gateway setup` interactive wizard isn't available (containers, SSH, headless), see `references/telegram-gateway-setup.md` for the three-location manual config procedure. Key gotcha: `hermes config set` fails on nested `gateway.telegram.*` keys — edit `config.yaml` directly.
...
- **Gateway denies all users**: Set `GATEWAY_ALLOW_ALL_USERS=true` in `~/.hermes/.env` or configure `TELEGRAM_ALLOWED_USERS` / platform-specific allowlists.
- **`hermes config set` fails on nested keys**: Dot-notation like `gateway.telegram.bot_token` raises `ValueError: Invalid environment variable name`. Edit `config.yaml` directly instead.
...
| Profiles | `hermes profile list` or [Profiles docs](https://hermes-agent.nousresearch.com/docs/user-guide/features/profiles) |
...
- **SOUL.md setup**: `references/soul-md-setup.md` — How to create and configure SOUL.md for autonomous agent identity
- **Enable all toolsets**: `references/enable-all-toolsets.md` — Quick reference for enabling all available toolsets
- **Telegram gateway setup**: `references/telegram-gateway-setup.md` — Manual Telegram gateway configuration
