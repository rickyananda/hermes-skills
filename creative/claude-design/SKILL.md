...
## Live-Site Design Extraction

When the user says "make it like [site]" or "ala ala [site]", browse the target and extract its design tokens (CSS variables, fonts, colors, spacing) via browser console before building. See `references/analyze-live-site.md` for the exact extraction workflow and code snippets.
